...
from lane_map import build_lanes, traffic_path, vehicle_route, build_pedestrian_lanes, pedestrian_point
import bootstrap
import collections
...
import traceback
import carla
from vehicle_lighting import AutomaticVehicleLights
from scene_vehicles import SceneVehicles, MOTORCYCLE_PRESETS
import numpy as np
...
from sensor_loadouts import DEFAULT_SENSORS, spawn_loadout
...
class Controller(GpuResources):
    def __init__(self):
        ...
        self.watchdog=threading.Thread(target=reliability.watch_workers,args=(self,),daemon=True);self.watchdog.start()
            ...
            try:
                ...
                if not f.cancelled():
                    try:
                        ...
                        if self.recording:self.recording.event(action,payload,self.state['frame'],self.state['time'])
                        if not self.worker_fault:self.state['error']=None
                        f.set_result(result)
                    except Exception as e:
                        traceback.print_exc();self.state['error']=str(e);f.set_exception(e)
            except queue.Empty:pass
            try:
                if self.worker_fault and self.recording:self.fail(self.worker_fault)
                if self.state['phase']=='starting':self.poll_start()
                elif self.pending_restore and self.state['phase']=='connected':
    ...
    def command(self,action,p):
        ...
        if action=='reload-parking':
            ...
            if not annotations.get('parking_validation'):raise ValueError(annotations['parking_source'])
            # Run between ticks; keep actors, sensors, navigation and world time.
            self.map_data={**{k:v for k,v in self.map_data.items() if not k.startswith('parking_')},**annotations}
            dump(DATA/'map-cache.json',self.map_data);self.refresh()
            return {'selectable':len(annotations['parking_spaces']),'excluded':len(annotations.get('parking_excluded',[]))}
        if action in ('gpu-profile','gpu-benchmark'):
            self.require_edit()
            return self.apply_gpu_profile(str(p.get('profile','auto'))) if action=='gpu-profile' else self.benchmark_gpu_profile()
        if action=='run':
        ...
        if action=='pause':
            self.running=False;self.state['running']=False;self.reconcile_resources();return {'running':False}
        ...
        if action=='weather':
            ...
            try:
                ...
                # A synchronous world must tick to deliver weather to replicas.
                self.tick()
            except Exception as error:
                self.state['weather_application']={'stage':'failed','detail':str(error)}
                raise
            self.state['weather_application']={'stage':'applied','frame':self.state['frame'],'detail':f"Weather applied at frame {self.state['frame']}"}
        ...
        if action=='replay-stop':
            ...
            gpus=getattr(self,'last_gpus','auto')
        ...
        if action=='pedestrian-path':
            m=self.managed.get(int(p['id']))
            if not m or m['role']!='pedestrian':raise ValueError('Select a pedestrian')
            target=carla.Location(**pedestrian_point(self.map_data,p['point']))
            route=[xyz(v) for v in m['controller'].get_navigation_path(target)]
            return {'reachable':bool(route),'route':route,'destination':route[-1] if route else None}
        if action=='destination':
            # Executed by the owner between ticks. Recording captures this event
            # and the new route in the next state, without interrupting sensors.
            if self.mode!='live':raise ValueError('Stop replay before changing destinations')
            m=self.managed.get(int(p['id']))
            if m and m['role']=='pedestrian' and 'arrival_tolerance' in p:
                tolerance=float(p['arrival_tolerance'])
                if not math.isfinite(tolerance) or not .25<=tolerance<=3:raise ValueError('Arrival tolerance must be 0.25–3 metres')
                m['arrival_tolerance']=tolerance
            return self.destination(int(p['id']),p['point'])
        self.require_edit()
        if action=='restore-configuration':self.restore_configuration(p['configuration']);return {'restored':True}
        if action=='spawn':return self.spawn(p)
        if action=='convert-scene-vehicles':return self.scene_vehicles.convert()
        if action=='parked-to-road':return self.scene_vehicles.release_to_road(int(p['id']),p['point'])
        if action=='delete':
            try:
                self.delete(int(p['id']))
                # Flush destruction and publish a snapshot without the actor.
                self.tick();self.tick()
            finally:self.refresh()
            if self.worker_manifest:self.reconcile_resources()
            return {'deleted':True}
        if action=='sensors':
    ...
    def poll_start(self):
        ...
        for path in manifests[:8]:
            ...
            if m.get('manager_pid')==self.proc.pid and m.get('ready'):
                ...
                self.connect();self.resource_snapshot();return
        time.sleep(0.5)

    def connect(self):
        self.scene_vehicles=None;self.state.pop('scene_vehicles',None)
        self.movements=None;self.signal_metadata={};self.worker_fault=None;self.signal_audit=SignalAudit()
    ...
    def build_map(self):
        ...
        self.map_data.update(build_pedestrian_lanes(self.wmap,self.opendrive))
        self.map_data.update(parking.build_parking(self.wmap,self.opendrive,DATA/'parking'/(self.wmap.name.split('/')[-1]+'.json')))
        self.parking_static=parking.static_vehicles(self.world,carla)
    ...
    def spawn(self,p,advance=True):
        if (p.get('saved_offroad_pose') or (p.get('scene_source') and p.get('planner')=='parked')) and p.get('role')!='ego':
            self.require_edit()
            actor=self.scene_vehicles.spawn(p['model'],p['spawn'],source=p.get('scene_source'),substituted=p.get('model_substituted',False),saved_pose=True)
            self.managed[actor.id]['parking_space']=p.get('parking_space')
            self.tick();self.refresh()
            if p.get('destination'):self.destination(actor.id,p['destination'])
            return {'id':actor.id,'actor':self.state['managed'][str(actor.id)]}
        if p.get('parking_space') and not (p.get('role')=='ego' and p.get('_restore_exact_pose')):
            if not advance:raise ValueError('Place parked vehicles while paused')
            result=self.spawn_parked(p)
            if p.get('destination'):self.destination(result['id'],p['destination'])
            return result
        ...
        if role=='pedestrian':
            ...
            loc=carla.Location(**pedestrian_point(self.map_data,p['spawn']));loc.z+=1
            ...
            if p.get('_restore_exact_pose'):
                # A saved driven vehicle may be between lane centres. Projecting
                # it onto a lane can move it into parked traffic or map geometry.
                point=p['spawn']
                t=carla.Transform(carla.Location(**{k:float(point[k]) for k in ('x','y','z')}),
                                  carla.Rotation(**{k:float(point.get(k,0)) for k in ('yaw','pitch','roll')}))
                t.location.z+=.2  # clearance for settled tyre/road contact
            else:
        ...
        if p.get('_restore_exact_pose'):self.managed[a.id]['_restore_hold']=True
        if p.get('scene_source'):self.managed[a.id].update(scene_source=p['scene_source'],model_substituted=p.get('model_substituted',False))
        if a.type_id in MOTORCYCLE_PRESETS:self.managed[a.id]['physics_preset']='approximate_motorcycle'
        try:
            ...
            if role=='pedestrian':
                ...
                if advance:
                    self.tick();ctrl.start();ctrl.set_max_speed(1.4)
                    if p.get('_restore_exact_pose'):ctrl.stop()
                else:self.scheduled_walkers.append({'id':a.id,'frame':self.world.get_snapshot().frame+1,'destination':p.get('destination')})
            else:
                a.set_autopilot((planner=='tm' or role=='background') and not p.get('_restore_exact_pose'),8005)
    ...
    def spawn_parked(self,p):
        self.require_edit()
        if p.get('role','background')!='background' or p.get('sensors'):raise ValueError('Parking placement is for background vehicles without sensors')
        if len(self.managed)>=100:raise ValueError('Limit: 100 managed actors')
        space=next((s for s in self.map_data.get('parking_spaces',[]) if s['id']==p['parking_space']),None)
        if space is None:raise ValueError('Choose a parking bay in the loaded map')
        self.refresh()
        if self.state['parking']['occupied'].get(space['id']):raise ValueError('That parking bay is occupied')
        if self.state['parking'].get('reserved',{}).get(space['id']):raise ValueError('That parking bay is reserved by an approaching vehicle')
        bp=self.world.get_blueprint_library().find(p['model'])
        if not bp.id.startswith('vehicle.'):raise ValueError('Choose a vehicle model')
        bp.set_attribute('role_name','parked')
        t=carla.Transform(carla.Location(x=space['x'],y=space['y'],z=space['z']+.5),carla.Rotation(yaw=space['yaw']))
        a=self.world.try_spawn_actor(bp,t)
        if not a:raise ValueError('The parking position is obstructed; choose another bay')
        try:
            e=a.bounding_box.extent
            if e.x*2+.04>space['length'] or e.y*2+.04>space['width']:
                raise ValueError(f'Vehicle needs {2*e.x+.04:.2f} × {2*e.y+.04:.2f} m including clearance; this bay is {space["length"]:.1f} × {space["width"]:.1f} m. Choose a smaller vehicle or wider bay')
            a.set_autopilot(False,8005)
            a.apply_control(carla.VehicleControl(brake=1.,hand_brake=True))
            self.managed[a.id]={'actor':a,'role':'background','planner':'parked','parked':True,'parking_space':space['id'],'destination':None,'route':[],'arrived':True,'last_control':0.,'controller':None}
            self.tick();self.tick();self.refresh()
        except Exception:
            self.managed.pop(a.id,None);a.destroy();self.refresh();raise
        return {'id':a.id,'parking_space':space['id'],'actor':self.state['managed'][str(a.id)]}
    ...
    def destination(self,aid,p):
        ...
        if m['role']!='pedestrian' and (m.get('parked') or m.get('parking_trip') or p.get('parking_space')):
            if not getattr(self,'parking_driver',None):self.parking_driver=ParkingDriving(self)
            return self.parking_driver.assign(aid,p)
        a=m['actor'];committed_junction=False
        if m['role']=='pedestrian':
            target=carla.Location(**pedestrian_point(self.map_data,p))
            route=[xyz(v) for v in m['controller'].get_navigation_path(target)]
            if not route:raise ValueError('No complete walkable route to that destination')
            target=carla.Location(**route[-1])
            if m.get('arrived'):
                m['controller'].start();m['controller'].set_max_speed(1.4)
            m['controller'].go_to_location(target);m['route_source']='CARLA navigation mesh';m['arrival_tolerance']=float(m.get('arrival_tolerance',.75))
        else:
            ...
            committed=None
            if m['planner']=='tm' and not m.get('arrived'):
                actions=self.tm.get_all_actions(a)
                if actions:committed=actions[0][1]
            path,committed_junction=vehicle_route(self.planner,a.get_location(),target,committed)
        ...
        update={'revision':m.get('route_update',{}).get('revision',0)+1,
                'frame':getattr(self,'state',{}).get('frame',0),
                'preserved_junction':committed_junction,
                'execution':'external' if m.get('planner')=='external' else 'navigation' if m['role']=='pedestrian' else 'traffic_manager'}
        m.update(destination=xyz(target),route=route,arrived=False,route_update=update);self.refresh()
        return {'route':route,'destination':xyz(target),'route_update':update,'arrived':False}
    ...
    def configure_sensors(self,aid,configs):
        ...
        if self.worker_manifest and not self.resizing_workers:
            ...
            count=desired_workers(all_configs,self.gpu_profile,self.gpu_benchmarks.get(self.profile_key(all_configs),{}).get('selected_workers'))
            if count!=self.state.get('worker_count'):
                return self.reallocate_sensors(loadouts,count)
        ...
        try:
            ...
            for s in new:self.sensors[s['actor'].id]=s
            if self.worker_manifest and not self.resizing_workers:self.resource_snapshot()
        except Exception:
            self.client.set_timeout(3)
            for s in new:
    ...
    def warm_sensor_views(self, sensors):
        ...
        # unstreamed textures. Camera captures need their own rendered frames;
        # warming workers before cameras exist does not warm these views.
        cameras=[s['actor'].id for s in sensors if s['type'].startswith('sensor.camera.')]
        if not cameras:
            self.tick(publish=False)
            return
        total=30
        self.state['camera_warmup']={'stage':'warming','sensor_ids':cameras,'completed':0,'total':total}
        self.state['gpu_operation']={'stage':'warming','detail':f'Warming camera views: 0/{total} frames','started':time.time()}
        try:
            for completed in range(1,total+1):
                if self.stop_event.is_set():raise RuntimeError('Camera warmup cancelled during shutdown')
                self.tick(publish=False)
                self.state['camera_warmup']['completed']=completed
                self.state['gpu_operation']['detail']=f'Warming camera views: {completed}/{total} frames'
        except Exception as error:
            self.state['camera_warmup']['stage']='failed'
            self.state['gpu_operation'].update(stage='failed',detail=f'Camera warmup failed: {error}')
            raise
        self.state['camera_warmup']['stage']='ready'
        self.state['gpu_operation'].update(stage='ready',detail=f'Camera views warmed over {total} synchronized frames')

    def fail(self,error):
        self.running=False
        self.state.update(running=False,phase='error',error=str(error))
        if self.recording:
            if self.worker_fault and self.client:self.client.set_timeout(3)
    ...
    def prune_removed_actors(self):
        ...
        for aid in list(self.managed):
            if snapshot.has_actor(aid) or str(aid) not in self.state.get('managed',{}):continue
    ...
    def _tick(self,publish=True):
        ...
        self.state['tick_progress']={'stage':'controls','started':time.monotonic(),'frame':self.state.get('frame',0)+1}
        ...
        if self.mode=='live':
            for m in list(self.managed.values()):
                ...
                if actor_snapshot is None:
                    ...
                    continue
                if m.get('_restore_hold') and m['role']!='pedestrian':
                    forced_controls.append(carla.command.ApplyVehicleControl(a.id,carla.VehicleControl(brake=1.,hand_brake=True)))
                    continue
                if m.get('parking_trip'):
                    if not getattr(self,'parking_driver',None):self.parking_driver=ParkingDriving(self)
                    control=self.parking_driver.tick(m,actor_snapshot)
                    if control is not None:forced_controls.append(carla.command.ApplyVehicleControl(a.id,control))
                    continue
                ...
                if m['destination'] and not m.get('arrived'):
                    ...
                    if distance<(m.get('arrival_tolerance',.75) if m['role']=='pedestrian' else 3):
                        m['arrived']=True
        ...
        self.state['tick_progress']={'stage':'world','started':time.monotonic(),'frame':self.state.get('frame',0)+1}
        ...
        self.previews={s['actor'].id:(s['data'].frame,s['type'],s['data']) for s in samples}
    ...
    def refresh(self,snap=None):
        ...
        vehicle_light_states=self.world.get_vehicles_light_states()
        for a in self.world.get_actors():
            if not snap.has_actor(a.id):continue
            if a.type_id.startswith(('vehicle.','walker.pedestrian.','traffic.traffic_light')):
                ...
                if a.type_id.startswith('traffic.traffic_light'):
                    ...
                    if a.type_id.startswith('vehicle.'):item['light_state']=int(vehicle_light_states.get(a.id,carla.VehicleLightState.NONE))
                actors.append(item)
        weather=self.world.get_weather()
        for m in self.managed.values():
            if m['role']=='pedestrian':m['walking_state']=reliability.walking_state(m,m['actor'],snap.timestamp.elapsed_seconds)
        managed={str(aid):{k:trip_status(v) if k=='parking_trip' else v for k,v in m.items() if k not in ('actor','controller','last_control','_walking_progress','_restore_hold')} for aid,m in self.managed.items()}
        parking_state={'reserved':{(m.get('parking_trip',{}).get('bay') or (m.get('destination') or {}).get('parking_space')):aid for aid,m in self.managed.items() if (m.get('parking_trip',{}).get('bay') or (m.get('destination') or {}).get('parking_space')) and not m.get('parked')},'occupied':parking.occupancy((self.map_data or {}).get('parking_spaces',[]),getattr(self,'parking_static',[]),actors)}
        parking_state['excluded_occupied']=parking.occupancy((self.map_data or {}).get('parking_excluded',[]),[],[a for a in actors if self.managed.get(a['id'],{}).get('parked')])
        if getattr(self,'scene_vehicles',None):self.state['scene_vehicles']=self.scene_vehicles.snapshot()
    ...
    def delete(self,aid):
        ...
        if m['actor'].destroy() is False:raise RuntimeError(f'CARLA did not remove actor {aid}; it remains selected for retry')
        del self.managed[aid]
        if self.ros:self.ros.remove_prefix(f'/carla/ego_{aid}/')
    ...
    def restore_configuration(self,config):
        ...
        for actor in config['actors']:
            if getattr(self,'scene_vehicles',None) and self.scene_vehicles.was_removed(actor.get('scene_source')):continue
            payload={k:v for k,v in actor.items() if k!='id'}
            payload['_restore_exact_pose']=True;payload['destination']=None
            if payload.get('role')=='ego' and payload.get('planner')=='parked':payload['planner']='tm'
            result=self.spawn(payload);restored.append((result['id'],actor))
            if actor.get('role')=='ego' and (actor.get('parked') or (actor.get('parking_space') and actor.get('saved_offroad_pose') and not actor.get('destination'))):
                self.managed[result['id']].update(parked=True,arrived=True,parking_space=actor.get('parking_space'),parking_trip={'stage':'parked','bay':actor.get('parking_space'),'blocked':None})
            if actor.get('role')=='pedestrian':
                newest=next(reversed(self.managed));self.managed[newest]['arrival_tolerance']=float(actor.get('arrival_tolerance',.75))
        for gid,program in config.get('movement_programs',{}).items():
            if program.get('active'):self.movements.apply(dict(group_id=int(gid),operation='enable',**{k:program[k] for k in ('phases','yellow_time','all_red_time')}),self.state['time'])
        if config.get('network_timing'):self.movements.network.configure(config['network_timing'],self.state['time'])
        if config.get('authoring',{}).get('config'):self.schedule.configure(self.validate_schedule(config['authoring']['config']))
        ...
        for aid,saved in restored:
            ...
            if m['role']=='pedestrian':m['controller'].start();m['controller'].set_max_speed(1.4)
                ...
                m['actor'].set_autopilot(True,8005)
            if saved.get('destination'):self.destination(aid,saved['destination'])
        self.tick()

        self.running=False;self.refresh()
        if 'scene_vehicles' not in config and getattr(self,'scene_vehicles',None):self.scene_vehicles.convert()
        self.state['recovery_operation']={'stage':'ready','detail':'Saved configuration restored and paused; actor IDs and simulation time may differ'}
        self.checkpoint_at=0;reliability.checkpoint(self)
    ...
    def disconnect(self,stop_process=False):
        ...
        self.running=False
        if self.recording:self.finish_record('failed' if self.worker_fault else 'complete',self.worker_fault)
        if self.world and (not self.proc or self.proc.poll() is None):
            try:
                ...
                self.clear_managed()
                if getattr(self,'scene_vehicles',None):self.scene_vehicles.restore({})
                self.tm.set_synchronous_mode(False);self.world.apply_settings(self.original_settings)
        ...
        self.movements=None;self.world=None;self.client=None;self.tm=None;self.vehicle_lighting=AutomaticVehicleLights();self.mode='live';self.sensors={};self.managed={};self.previews={}
