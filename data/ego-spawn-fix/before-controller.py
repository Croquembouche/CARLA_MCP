...
class Controller(GpuResources):
    ...
    def spawn(self,p,advance=True):
        if p.get('saved_offroad_pose') or (p.get('scene_source') and p.get('planner')=='parked'):
            self.require_edit()
    ...
    def spawn_parked(self,p):
        ...
        try:
            ...
            if e.x*2+.1>space['length'] or e.y*2+.1>space['width']:
                raise ValueError(f'Vehicle needs {2*e.x+.1:.1f} × {2*e.y+.1:.1f} m including clearance; this bay is {space["length"]:.1f} × {space["width"]:.1f} m. Choose a smaller vehicle or wider bay')
    ...
    def refresh(self,snap=None):
        ...
        parking_state={'reserved':{m['parking_trip']['bay']:aid for aid,m in self.managed.items() if m.get('parking_trip',{}).get('bay') and not m.get('parked')},'occupied':parking.occupancy((self.map_data or {}).get('parking_spaces',[]),getattr(self,'parking_static',[]),actors)}
    ...
    def export_config(self):
        ...
        return {'version':4,'scene_vehicles':{'hidden':sorted(self.scene_vehicles.hidden)} if getattr(self,'scene_vehicles',None) else {},'authoring':self.schedule.snapshot(),'network_timing':self.movements.network.snapshot() if self.movements else {},'map':self.state['map'],'weather':self.state.get('weather'),'movement_programs':self.movements.snapshot() if self.movements else {},
                ...
                'actors':[dict(id=aid,model=m['actor'].type_id,role=m['role'],saved_offroad_pose=bool(m.get('parked') or m.get('parking_trip',{}).get('stage') in ('leaving','entering','blocked')),planner='parked' if m.get('parked') or m.get('parking_trip',{}).get('stage') in ('leaving','entering','blocked') else m['planner'],parking_space=m.get('parking_space'),scene_source=m.get('scene_source'),model_substituted=m.get('model_substituted',False),spawn=poses[aid],destination=None if m.get('parked') else m['destination'],
