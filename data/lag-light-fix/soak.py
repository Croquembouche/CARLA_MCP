...
try:
 while True:
  ...
  row={'seconds':round(now-started,2),'frame':f,'http_ms':round(http_ms,2),'rss_gib':rss,'gap_seconds':round(now-last_advance,2),'timings':s.get('performance',{}).get('last_frame_ms'),'health':s.get('stream_health')};records.append(row)
  if len(records)%15==1:print(json.dumps({k:v for k,v in row.items() if k!='timings'}),flush=True)
 ...
 cmd('pause');(out/'soak.json').write_text(json.dumps({'start_frame':start,'last_frame':last_frame,'max_observed_gap_seconds':max_gap,'samples':records},indent=2))
