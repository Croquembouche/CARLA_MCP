...
@app.get('/api/status')
# Encode the built-in snapshot immediately; FastAPI's recursive Python encoder
# could otherwise iterate while the simulator thread adds progress fields.
def status():return JSONResponse(engine.state)
...
def preview(sensor_id:int,request:Request,width:int=Query(640,ge=160,le=960),after:int=Query(-1,ge=-1)):
    item=engine.previews.get(sensor_id)
    if not item or not any(s['id']==sensor_id for s in engine.state.get('sensors',[])):
    ...
    etag=f'"{sensor_id}-{frame}-{data.timestamp}-{width}-{id(data)}"'
    ...
    try:body,mime,extra=preview_cache.get(etag,kind,data,width)
