...
class Controller(GpuResources):
    ...
    def refresh(self,snap=None):
        ...
        managed={str(aid):{k:trip_status(v) if k=='parking_trip' else v for k,v in m.items() if k not in ('actor','controller','last_control','_walking_progress')} for aid,m in self.managed.items()}
