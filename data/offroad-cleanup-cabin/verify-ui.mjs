...
try{
 await page.goto('http://127.0.0.1:8095');await page.waitForFunction(()=>document.querySelector('#actor-count').textContent==='35');
 const dismiss=page.getByRole('button',{name:'Continue to scene',exact:true});if(await dismiss.isVisible())await dismiss.click();
 await page.locator('nav [data-panel="sensorview"]').click();await page.locator('#view-ego').selectOption('28');await page.waitForFunction(()=>[...document.querySelectorAll('#view-focus option')].some(o=>o.textContent==='cabin_overview'),null,{timeout:180000});
 ...
 await page.screenshot({path:'data/offroad-cleanup-cabin/cabin-ui.png',fullPage:true});assert.deepEqual(errors,[]);fs.writeFileSync('data/offroad-cleanup-cabin/ui-verification.json',JSON.stringify({ego:28,camera:'cabin_overview',imageLoaded:true,errors},null,2));console.log('Verified cabin_overview is visible with a live image on ego 28.');
