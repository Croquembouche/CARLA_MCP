...
class Controller(GpuResources):
    ...
    def spawn(self,p,advance=True):
        if p.get('scene_source') and p.get('planner')=='parked':
            self.require_edit()
            actor=self.scene_vehicles.spawn(p['model'],p['spawn'],source=p['scene_source'],substituted=p.get('model_substituted',False),saved_pose=True)
            self.tick();self.refresh();return {'id':actor.id,'actor':self.state['managed'][str(actor.id)]}
        if p.get('parking_space'):
            if not advance:raise ValueError('Place parked vehicles while paused')
            return self.spawn_parked(p)
    ...
    def destination(self,aid,p):
        ...
        if m.get('parked'):raise ValueError('This vehicle is parked off the driving network. Pause, select a Road start point and use Move to road & enable Traffic Manager before assigning a destination')
    ...
    def refresh(self,snap=None):
        ...
        managed={str(aid):{k:v for k,v in m.items() if k not in ('actor','controller','last_control','_walking_progress')} for aid,m in self.managed.items()}
        parking_state={'occupied':parking.occupancy((self.map_data or {}).get('parking_spaces',[]),getattr(self,'parking_static',[]),actors)}
    ...
    def export_config(self):
        ...
        return {'version':4,'scene_vehicles':{'hidden':sorted(self.scene_vehicles.hidden)} if getattr(self,'scene_vehicles',None) else {},'authoring':self.schedule.snapshot(),'network_timing':self.movements.network.snapshot() if self.movements else {},'map':self.state['map'],'weather':self.state.get('weather'),'movement_programs':self.movements.snapshot() if self.movements else {},
                ...
                'actors':[dict(id=aid,model=m['actor'].type_id,role=m['role'],planner=m['planner'],parking_space=m.get('parking_space'),scene_source=m.get('scene_source'),model_substituted=m.get('model_substituted',False),spawn=poses[aid],destination=m['destination'],
