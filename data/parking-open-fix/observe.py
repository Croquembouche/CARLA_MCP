...
c.post('/api/command/run',json={}).raise_for_status();rows=[];last=None;start=time.monotonic();sensors=sorted(v['id'] for v in s['sensors'])
while time.monotonic()-start<500:
 ...
 if m.get('parked'):
  c.post('/api/command/pause',json={}).raise_for_status();root.joinpath('r135-completed.json').write_text(json.dumps({'actor':a,'managed':m,'sensor_ids':sensors,'after_sensor_ids':sorted(v['id'] for v in s['sensors'])},indent=2));print('PARKED',flush=True);break
 if row['stage']=='blocked' or s.get('phase')=='error':print('FAILED',flush=True);break
 time.sleep(.6)
else:print('TIMEOUT',json.dumps(rows[-1]),flush=True)
root.joinpath('r135-samples.json').write_text(json.dumps(rows,indent=2))
