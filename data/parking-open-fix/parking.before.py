    ...
    if 'validated_spaces' in data:
        ...
        if validation.get('geometry_sha256'):
            ...
            if (not geometry.exists() or not scene.exists()
                ...
                return result
        if rules:
            scene_source=source.parent.parent/'scene-source'/'scene.json'
            if (rules.get('opendrive_sha256')!=hashlib.sha256(opendrive.encode()).hexdigest()
                or not scene_source.exists()
                or rules.get('scene_sha256')!=hashlib.sha256(scene_source.read_bytes()).hexdigest()):
                result['parking_source']='Parking restrictions require revalidation for this scene revision'
                return result
            spaces,excluded=screen_spaces(spaces,rules)
        result.update(parking_spaces=spaces,parking_areas=data.get('areas',[]),
                      parking_curb_strips=data.get('curb_strips',[]),
                      parking_source=f"{len(spaces)} selectable curb parking positions; {len(excluded)} restricted or unresolved positions withheld. Curb and paint edges are surveyed where available; unpainted divisions between cars remain planning estimates.",
                      parking_validation=data['validation'],parking_excluded=excluded,
                      parking_rules={k:v for k,v in (rules or {}).items() if k!='zones'})
        return result
    samples=[]
