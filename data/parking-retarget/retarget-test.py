...
def drive_road(a,target,replace=None):
 ...
 result={'case':'replace_during_reverse_leaving_and_road','trace':[],'arrived':False};report['cases'].append(result)
 ...
 try:
  for i in range(1600):
   ...
   c.tick()
   if i in (80,140):
    old=m['parking_trip'];old_revision=m['route_update']['revision'];old_goal=dict(m['destination'])
    wp=c.waypoint(point(a.get_transform()));dest=point(wp.next(35 if i==80 else 15)[0].transform)
    new=c.driver.assign(a.id,{k:dest[k] for k in ('x','y','z')})
    assert m['parking_trip'] is not old and not m['arrived'] and new['route_update']['revision']==old_revision+1
    result.setdefault('replacements',[]).append({'frame':i,'previous_stage':old['stage'],'old_goal':old_goal,'new_goal':new['destination'],'revision':new['route_update']['revision']})
    print('REPLACED',result['replacements'][-1],flush=True)
   if i%50==0:
  ...
  out.write_text(json.dumps(report,indent=2));assert result['arrived'] and not collisions and len(result.get('replacements',[]))==2,result
 finally:sensor.stop();sensor.destroy()
try:
 ...
 a=c.spawn(start.transform);created.append(a)
 c.driver.assign(a.id,{'parking_space':'P025'});m=c.managed[a.id]
 for i in range(1800):
  ctrl=c.driver.tick(m,c.world.get_snapshot().find(a.id))
  if ctrl is not None:c.client.apply_batch_sync([carla.command.ApplyVehicleControl(a.id,ctrl)],False)
  c.tick()
  if m['parking_trip']['stage']=='entering' and a.get_location().x>11 and a.get_control().reverse:
   print('INTERRUPTING REVERSE',point(a.get_transform()),flush=True);break
 else:raise AssertionError('Never reached reversing phase')
 assert not m.get('arrived')
