...
OUT=ROOT/'data/quality-benchmark'; c=httpx.Client(base_url='http://127.0.0.1:8095',timeout=15)
...
initial=status();save('initial-status.json',initial);cmd('pause');config=c.get('/api/configuration').json();save('original.json',config)
# Copy the exact original settings before any experiment.
try:
 if not (OUT/'baseline-high.json').exists():measure('baseline-high',60)
 cmd('run')
 for port in [2010,2020,2030,2040]:
  for command in MAX:
   print('SETTING',port,command,flush=True);console(port,command)
 save('epic-settings.json',MAX)
 cmd('pause')
 for name,width,height in [('epic-720p',1280,720),('epic-1080p',1920,1080)]:
  s=status();ego=next(int(aid) for aid,m in s['managed'].items() if m['role']=='ego')
  original_ego=next(a for a in config['actors'] if a['role']=='ego');loadout=copy.deepcopy(original_ego['sensors'])
  for sensor in loadout:
   if sensor['type']=='sensor.camera.rgb':sensor.setdefault('attributes',{}).update(image_size_x=str(width),image_size_y=str(height),use_ray_tracing='false')
  cmd('sensors',{'id':ego,'sensors':loadout});save(name+'-configuration.json',c.get('/api/configuration').json())
  measure(name,90)
except BaseException as e:
 save('failure.json',{'error':repr(e)});print('BENCHMARK FAILURE',repr(e),flush=True)
finally:
 print('RESTORING original scene and quality',flush=True)
 ...
 save('restored-status.json',status());print('RESTORED',flush=True)
