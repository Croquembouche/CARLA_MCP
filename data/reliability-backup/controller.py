...
class Controller(GpuResources):
    ...
    def _tick(self,publish=True):
        ...
        row={'control_ms':(before_world-timing_start)*1000,'world_ms':(after_world-before_world)*1000,
             ...
             'publish_record_ms':(after_publish-after_refresh)*1000,'preview_ms':(ended-after_publish)*1000,
             'total_ms':(ended-timing_start)*1000}
        self.frame_timings.append(row)
        self.state['performance']={'frames':len(self.frame_timings),'timings_ms':summary(list(self.frame_timings)),
