...
class GpuResources:
    ...
    def profile_key(self, configs):
        context={'renderer_version':'demand-v1','sensors':loadout_key(configs),'map':self.state.get('map'),'weather':self.state.get('weather'),
    ...
    def benchmark_gpu_profile(self):
        ...
        try:
            for count in range(1,maximum+1):
                ...
                stats=summary(list(self.frame_timings))
                results.append({'workers':count,'frames':50,'timings_ms':stats,'memory':self.process_memory()})
            ...
            report={'selected_workers':selected,'results':results,'measured_at':time.time(),
                    'selection':'Fewest workers within 10% of lowest mean frame time; 50 frames per profile'}
