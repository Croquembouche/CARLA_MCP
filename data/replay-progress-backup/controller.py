...
class Controller:
    ...
    def command(self,action,p):
        if action=='start':
            ...
            self.started=time.monotonic();self.state.update(phase='starting',error=None,worker_count=len(gpus.split(',')))
            return {'started':True}
        if action=='connect':
        ...
        if action=='replay-stop':
            ...
            gpus=getattr(self,'last_gpus','0,1,2,3')
    ...
    def connect(self):
        ...
        self.state.update(phase='connected',error=None,running=False,map=wmap.name,server_version=client.get_server_version())
        self.refresh()
    ...
    def spawn(self,p,advance=True):
        ...
        configs=validate_loadout(p.get('sensors',DEFAULT_SENSORS if role=='ego' else []))
        ...
        if self.mode=='live':
            for m in self.managed.values():
                ...
                if m['destination'] and not m.get('arrived'):
                    d=m['destination'];loc=a.get_location()
    ...
    def export_config(self):
        return {'version':3,'authoring':self.schedule.snapshot(),'network_timing':self.movements.network.snapshot() if self.movements else {},'map':self.state['map'],'weather':self.state.get('weather'),'movement_programs':self.movements.snapshot() if self.movements else {},
                ...
                'actors':[dict(id=aid,model=m['actor'].type_id,role=m['role'],planner=m['planner'],spawn=pose(m['actor'].get_transform()),destination=m['destination'],
                               sensors=[s['config'] for s in self.sensors.values() if s['parent']==aid]) for aid,m in self.managed.items()],
