...
class Owner:
 ...
 def spawn(self,t,model='vehicle.mini.cooper'):
  ...
  return a
c=Owner();report={'cases':[]};out=Path('data/reverse-parking/native-verification.json');created=[]
def run(a,bid):
...
try:
 ...
 choices=near.previous(60);start=next(w for w in choices if not w.is_junction and abs(angle(math.radians(w.transform.rotation.yaw-b['yaw'])))<.2)
 a=c.spawn(start.transform);created.append(a);run(a,'P025');run(a,'P024')
finally:
 for a in created:
