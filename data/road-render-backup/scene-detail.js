 ...
 for(const batch of meta.groups){
  ...
  for(const {geometry,slot} of geometries.get(batch.mesh)){
   ...
   if(!materials.has(materialKey)){
    ...
    const material=new THREE.MeshLambertMaterial({color,map:texture,clippingPlanes,alphaTest:texture&&(info?.browser_alpha||(meta.category==='vegetation'&&info?.blend?.includes('MASKED')))?.4:0,side:THREE.DoubleSide});materials.set(materialKey,material);
   }
