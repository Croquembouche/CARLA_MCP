...
class Controller(GpuResources):
    ...
    def command(self,action,p):
        ...
        if action=='delete':
            self.delete(int(p['id']));self.refresh()
    ...
    def destination(self,aid,p):
        ...
        if m.get('parked'):raise ValueError('This vehicle is parked off the driving network. Remove it while paused, then spawn a Traffic Manager vehicle on a road to drive')
    ...
    def export_config(self):
        ...
        return {'version':3,'authoring':self.schedule.snapshot(),'network_timing':self.movements.network.snapshot() if self.movements else {},'map':self.state['map'],'weather':self.state.get('weather'),'movement_programs':self.movements.snapshot() if self.movements else {},
                ...
                'actors':[dict(id=aid,model=m['actor'].type_id,role=m['role'],planner=m['planner'],parking_space=m.get('parking_space'),spawn=poses[aid],destination=m['destination'],
    ...
    def disconnect(self,stop_process=False):
        ...
        self.state.update(worker_count=0,gpu_workers=[],phase='offline',mode='live',running=False,actors=[],managed={},sensors=[],recording=None,error=None)
