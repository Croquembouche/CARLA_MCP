...
class ParkingDriving:
    ...
    def bay(self,aid,bid):
        ...
        if m['role'] not in ('background','ego'):raise ValueError('Parking destinations require a vehicle')
        e=a.bounding_box.extent
        if e.x*2+.04>space['length'] or e.y*2+.04>space['width']:raise ValueError(f'Vehicle needs {2*e.x+.04:.2f} × {2*e.y+.04:.2f} m; bay {bid} is {space["length"]:.1f} × {space["width"]:.1f} m')
        for otherid,other in o.managed.items():
            if otherid!=aid and (other.get('parking_trip',{}).get('bay') or (other.get('destination') or {}).get('parking_space'))==bid:raise ValueError(f'Bay {bid} is reserved by vehicle {otherid}')
    ...
    def assign_external(self,aid,p):
        """Publish a validated parking goal without taking over an external ego."""
        o=self.owner;m=o.managed[aid];a=m['actor'];space=self.bay(aid,p['parking_space'])
        near=o.wmap.get_waypoint(carla.Location(x=space['x'],y=space['y'],z=space['z']),lane_type=carla.LaneType.Driving)
        if not near or near.is_junction:raise ValueError('This bay has no non-junction driving-lane approach')
        path,_=vehicle_route(o.planner,a.get_location(),near.transform.location,None)
        target={k:space[k] for k in ('x','y','z')};target.update(yaw=near.transform.rotation.yaw,parking_space=space['id'])
        route=[dict(x=w.transform.location.x,y=w.transform.location.y,z=w.transform.location.z) for w,_ in path]
        route.append({k:target[k] for k in ('x','y','z')})
        update=dict(revision=m.get('route_update',{}).get('revision',0)+1,frame=o.state.get('frame',0),preserved_junction=False,execution='external')
        m.update(destination=target,route=route,route_update=update,arrived=False)
        o.refresh();return dict(destination=target,route=route,route_update=update,arrived=False,planner='external')
    def assign(self,aid,p):
        o=self.owner;m=o.managed[aid]
        if m['role']=='ego' and m.get('planner')=='external':return self.assign_external(aid,p)
        a=m['actor'];start=point(a.get_transform());wb,steer,radius=self.geometry(a);obstacles=self.obstacles(a)
        ...
        if space:
            ...
            for approach in (7.,4.,2.,0.,10.,12.,max(14.,radius*3),18.):
                choices=near.next(approach) if approach else [near]
                if not choices or choices[0].is_junction:continue
                try:
                    road_goal=choices[0];entry=parking_entry(point(road_goal.transform),target,obstacles,radius,wb,self.rear_axles[a.id])
                    self.check_junctions(entry)
                    break
                except ValueError as e:errors.append(str(e))
            else:raise ValueError(f'No clear approach for bay {bid}: '+(errors[-1] if errors else 'no available staging lane'))
        ...
        if space and (m.get('parked') or on_road) and distance(start,target)<25 and closest.road_id==near.road_id and closest.section_id==near.section_id and closest.lane_id*near.lane_id>0 and not closest.is_junction:
        ...
        # A parked actor may retain handbrake/neutral after physics wakes.
        # Bypass Actor.apply_control's cache (ticks use batch controls too),
        # release the parking brake and explicitly choose the next gear.
        # While moving, retain the current gear until tick() stops for a change.
        current=a.get_control();local=exit_path or (entry if direct else [])
        reverse=local[0]['reverse'] if local else False
        if a.get_velocity().length()>.12:reverse=current.reverse
        control=carla.VehicleControl(brake=1,hand_brake=False,reverse=reverse,manual_gear_shift=True,gear=-1 if reverse else 1)
        for response in o.client.apply_batch_sync([carla.command.ApplyVehicleControl(a.id,control)],False):
            if response.error:raise RuntimeError('Cannot release parked vehicle: '+response.error)
    ...
    def tick(self,m,snapshot):
        ...
        if t['stage']=='driving':
            ...
            if t['entry'] and lane is not None and not lane.is_junction:
                # TM custom paths can finish in an adjacent lane on the same
                # carriageway. The local controller can cross that lane after
                # validating the swept path; it must not wait for an exact lane
                # match, or take over from opposing traffic / another road.
                on_target_lane=on_target_lane or (lane.road_id==t['road_goal_lane'][0] and lane.section_id==t['road_goal_lane'][1] and lane.lane_id*t['road_goal_lane'][2]>0)
            if d<25 and on_target_lane:o.tm.set_desired_speed(a,max(3.,min(18.,3.6*math.sqrt(max(.1,2*(d-1.))))))
            if not on_target_lane:return None
            if d>2.5:
                # TM may stop short of the nominal staging point (e.g. a signal
                # trigger farther ahead). Park from the current pose when a
                # checked local path already reaches this bay, rather than wait
                # indefinitely for an unnecessary drive past it.
                now=o.world.get_snapshot().timestamp.elapsed_seconds
                if not t['entry'] or lane.is_junction or speed>1 or distance(p,t['target'])>14 or d>12 or now<t.get('next_handoff_check',0):return None
                t['next_handoff_check']=now+2
                try:
                    candidate=parking_entry(p,t['target'],self.obstacles(a),self.geometry(a)[2],t['wheelbase'],t.get('rear_axle'))
                    self.check_junctions(candidate)
                except ValueError as error:
                    t['blocked']='Waiting for a clear parking handoff: '+str(error)
                    return None
                t['entry']=candidate
            a.set_autopilot(False,self.tm_port)
            if not t['entry']:
        ...
        # A five-metre bay may leave only centimetres beyond the body. The
        # previous fixed 18 cm stop threshold could never satisfy its footprint
        # check, even after replanning the final alignment.
        stop_tolerance=.18
        if t['stage']!='leaving' and end==len(path)-1:
            heading_error=angle(math.radians(p['yaw']-t['target']['yaw']))
            extent=a.bounding_box.extent
            projected_half=extent.x*abs(math.cos(heading_error))+extent.y*abs(math.sin(heading_error))
            stop_tolerance=min(.18,max(.005,t['target']['length']/2-projected_half-.01))
        reached=remaining<.4 if t['stage']=='leaving' else along_to_end<stop_tolerance and remaining<.6
        ...
        target_speed=min(1.2 if not reverse else .8,max(.10 if end==len(path)-1 and t['stage']!='leaving' else .25,remaining*.7))
