...
if s.get('recording') or s.get('mode','live')!='live':raise RuntimeError('Active recording or replay; do not restart')
config=c.get('/api/configuration').json() if s.get('map') else json.loads((data/'before-signal-controls-config.json').read_text())
(data/'movement-restore-config.json').write_text(json.dumps(config,indent=2))
...
cmd('weather',config['weather']);(data/'movement-restored-ids.json').write_text(json.dumps(mapping,indent=2))
(data/'movement-deployed-state.json').write_text(json.dumps(c.get('/api/status').json(),indent=2));print('RESTORE_COMPLETE',flush=True)
