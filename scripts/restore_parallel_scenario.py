...
root=Path(__file__).resolve().parents[1];data=root/'data';config=json.loads((data/'parallel-backup-config.json').read_text())
...
(data/'parallel-restored-ids.json').write_text(json.dumps(ids,indent=2));(data/'parallel-restored-state.json').write_text(json.dumps(s,indent=2));print('RESTORE_COMPLETE',s['frame'],len(s['sensors']),flush=True)
