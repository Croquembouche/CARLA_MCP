"""Restore the saved scenario after resource and signal validation."""
import json,time,httpx
from pathlib import Path
root=Path(__file__).resolve().parents[1];data=root/'data';config=json.loads((data/'resource-restore-config.json').read_text())
...
(data/'resource-restored-ids.json').write_text(json.dumps(ids,indent=2));(data/'resource-restored-state.json').write_text(json.dumps(s,indent=2));print('RESTORE_COMPLETE',s['frame'],len(s['sensors']),flush=True)
