...
actor=None;report={'lane':lane,'spawn':start,'goal':goal,'arrival_tolerance_m':.75}
try:
 actor=cmd('spawn',{'role':'pedestrian','model':'walker.pedestrian.0043','spawn':start,'destination':goal})['id'];report['actor']=actor
 route=cmd('pedestrian-path',{'id':actor,'point':goal});assert route['reachable'] and len(route['route'])>=2;report['route_preview']=route
 for phase,target in [('outbound',goal),('return',start)]:
  ...
  assert math.hypot(after['x']-target['x'],after['y']-target['y'])<.8,report
 ...
 (root/'data/reliability-walking-test.json').write_text(json.dumps(report,indent=2))
