...
from physical_lidar import xyzi
...
def encode_points(kind,data,limit=4000,color="height"):
    if color not in ("height","intensity"):raise ValueError("Unknown LiDAR color mode")
    ...
    if kind.endswith('ray_cast_semantic'):
        ...
        raw=xyzi(data);xyz=raw[:,:3];tags=None
    ...
    if tags is not None:colours=PALETTE[tags[::step][valid]%len(PALETTE)]
        ...
        t=np.clip(raw[::step,3][valid],0,1) if color=='intensity' else np.clip((xyz[:,2]+2)/6,0,1)
        colours=intensity_colours(t) if color=='intensity' else np.column_stack((60+180*t,220-100*t,240-160*t)).astype(np.uint8)
    packed=np.empty(len(xyz),dtype=np.dtype([('xyz','<i2',(3,)),('rgb','u1',(3,))]))
    packed['xyz']=np.rint(xyz/scale).astype('<i2');packed['rgb']=colours
    return packed.tobytes(),'application/vnd.carla.pointcloud',{'X-Preview-Points':str(len(xyz)),'X-Preview-Source-Points':str(total),'X-Point-Scale':str(scale),'X-Point-Stride':'9','X-Preview-Color':('semantic' if tags is not None else color)}

def intensity_colours(values):
    # Fixed 0..1 scale: dark blue is weak, yellow is strong. No per-frame auto gain.
    t=np.nan_to_num(np.clip(values,0,1));return np.column_stack((25+230*t,35+205*t,100-70*t)).astype(np.uint8)

def encode_preview(kind,data,width=640,format="image",point_limit=4000,color="height"):
    if color not in ("height","intensity"):raise ValueError("Unknown LiDAR color mode")
    if format=="points":return encode_points(kind,data,point_limit,color)
    ...
    if kind.startswith('sensor.camera.'):
        ...
        if kind=='sensor.other.radar':
            ...
            raw=xyzi(data);xyz=raw[:,:3];colour=raw[:,3] if color=='intensity' else raw[:,2];legend='Intensity: dark blue 0 / yellow 1' if color=='intensity' else 'Colour: height'
        ...
        if kind.endswith('ray_cast_semantic'):colours=PALETTE[colour.astype(int)%len(PALETTE)]
            ...
            t=np.clip((colour+2)/6,0,1);colours=intensity_colours(colour) if color=='intensity' else np.column_stack((60+180*t,220-100*t,240-160*t)).astype(np.uint8)
