...
export function markingLines(map){const out=[];for(const m of map.markings||[]){const a=m.yaw*Math.PI/180,c=Math.cos(a),s=Math.sin(a);for(const line of glyph(m))for(let i=1;i<line.length;i++){for(const p of [line[i-1],line[i]])out.push(m.x+p[0]*c-p[1]*s,m.z+.30,m.y+p[0]*s+p[1]*c)}}return out}
