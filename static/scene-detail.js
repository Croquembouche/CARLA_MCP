 ...
 for(const batch of meta.groups){
  ...
  for(const {geometry,slot} of geometries.get(batch.mesh)){
   ...
   for(const tile of tiles.values()){
    ...
    tile.forEach((t,i)=>{o.position.fromArray(t);o.quaternion.fromArray(t,3);o.scale.fromArray(t,7);o.updateMatrix();inst.setMatrixAt(i,o.matrix)});inst.computeBoundingSphere();inst.userData.sceneLayer=meta.category;
...
export class SceneDetail{
 ...
 setHiddenVehicles(sources){const key=JSON.stringify(sources.map(s=>[s.key,s.x,s.y,s.z,s.yaw,s.length,s.width,s.height]));if(key===this.vehicleKey)return;this.vehicleKey=key;this.hiddenVehicles=sources;this.applyHiddenVehicles();this.view.render()}
 applyHiddenVehicles(){const zero=new THREE.Matrix4().makeScale(0,0,0);this.group.traverse(o=>{if(!o.userData.vehicleInstances)return;o.userData.vehicleInstances.forEach((entry,i)=>{const p=entry.center;const hidden=(this.hiddenVehicles||[]).some(s=>{const a=s.yaw*Math.PI/180,dx=p.x-s.x,dy=p.z-s.y;return Math.abs(dx*Math.cos(a)+dy*Math.sin(a))<s.length/2+.3&&Math.abs(-dx*Math.sin(a)+dy*Math.cos(a))<s.width/2+.3&&Math.abs(p.y-s.z)<s.height/2+.5});o.setMatrixAt(i,hidden?zero:entry.matrix)});o.instanceMatrix.needsUpdate=true})}
