...
export class SensorView{
 constructor(root,controls){
  this.root=root;this.controls=controls;this.active=false;this.cards=new Map();this.selected=new Set();this.mode='all';this.fps=5;this.inFlight=0;this.transfers=[];this.state={};this.orbits=new Map();
  root.innerHTML=`<div class="sensor-view-heading"><div><div class="eyebrow">LIVE OBSERVATION</div><h2>Sensor views</h2></div><div id="sensor-transfer" role="status">No preview transfer</div></div><div class="sensor-view-toolbar"><label>Ego vehicle<select id="view-ego" aria-label="View ego vehicle"></select></label><div class="segmented"><button id="view-all" class="active">All sensors</button><button id="view-single">Single sensor</button></div><label id="view-focus-label" hidden>Sensor<select id="view-focus"></select></label><label>Preview rate<select id="view-rate"><option value="2">2 fps · low bandwidth</option><option value="5" selected>5 fps · balanced</option><option value="10">10 fps · smoother</option></select></label><button id="view-freeze">Freeze previews</button></div><p id="sensor-view-status" class="hint" role="status"></p><p id="sensor-scene-weather" class="hint" role="status"></p><div id="sensor-grid"></div>`;
 ...
 async fetchCard(c){
  ...
  try{
   const r=await fetch(`/api/preview/${c.sensor.id}?width=${width}&after=${c.lastFrame}${c.sensor.type.startsWith('sensor.lidar.')?'&format=points&point_limit='+(this.mode==='single'?12000:4000):''}`,{signal:abort.signal,cache:'no-store'});
   ...
   if(r.headers.has('X-Preview-Points'))c.node.querySelector('.sensor-quality').textContent=`${Number(r.headers.get('X-Preview-Points')).toLocaleString()} / ${Number(r.headers.get('X-Preview-Source-Points')).toLocaleString()} points · ${c.sensor.type.startsWith('sensor.lidar.')?'drag to rotate · scroll to zoom':'top view'}`;
