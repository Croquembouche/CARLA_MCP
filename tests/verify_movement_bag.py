...
import numpy as np
root=Path(__file__).resolve().parents[1]; report=json.loads((root/'data/movement-verification/report.json').read_text());session=root/'data/recordings'/report['recording']['id'];report=json.loads((session/'manifest.json').read_text())
...
(root/'data/movement-verification/bag-report.json').write_text(json.dumps(result,indent=2));print(json.dumps(result))
