...
def config(states):
 cmd('movement-program',{'group_id':18,'operation':'enable','yellow_time':.5,'all_red_time':.5,'phases':[{'name':'Driving verification','duration':60,'states':states}]})
 for _ in range(40):
  cmd('step')
  if state()['movement_programs']['18']['stage']=='green':break
 else:raise AssertionError('Phase did not leave clearance')
 cmd('movement-program',{'group_id':18,'operation':'hold'})
...
try:
 cmd('pause');config({'18':{'right':'Stop'}})
 left=spawn(18,'right');red=[]
 for _ in range(100):cmd('step');red.append(sample(left))
 assert any(x['light']==18 and x['at_light'] and x['speed']<.1 and x['brake']>.5 for x in red),red[-1]
 ...
 config({'18':{'right':'Permissive'},'19':{'straight':'Protected'}})
 opposing=spawn(19,'straight')
 entry=signal(19)['movement_lanes']['straight']['paths'][0][0]
 for _ in range(200):
  cmd('step')
  if wm.get_waypoint(opposing.get_location()).is_junction and math.hypot(opposing.get_location().x-entry[0],opposing.get_location().y-entry[1])>15:break
 left=spawn(18,'right',6);trace=[]
 for _ in range(260):
 ...
 report['permissive_right']={'yield_frames':len(yielding),'left_entry':left_entry,'protected_opposing_entry':opposing_entry,'released_after_yield':True}
 (OUT/'right-driving-trace.json').write_text(json.dumps({'red':red,'trace':trace},indent=2))
 for aid in list(created):cmd('delete',{'id':aid});created.remove(aid)
 config({'18':{'right':'Protected'}})
 protected=spawn(18,'right',6);protected_trace=[]
 for _ in range(120):
  cmd('step');protected_trace.append(sample(protected))
  if protected_trace[-1]['junction']:break
 assert any(x['junction'] for x in protected_trace)
 report['protected_right_entered']=True
 report['result']='passed';(OUT/'right-driving-report.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2),flush=True)
finally:
 for aid in created:
